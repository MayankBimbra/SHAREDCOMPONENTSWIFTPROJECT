...
protocol VACalendarDelegateEmpty: class {
...
public class VACalendar {
    ...
    weak var delegateEmpty : VACalendarDelegateEmpty?
    ...
    func getSelecteddays(_ day : VADay, state: VADayState){
//        delegate?.selectedDaysDidUpdate(selectedDays)
    ...
    func removeDays(_ day: VADay){
        if selectedDays.contains(day){
            for (kNum, selDay) in selectedDays.enumerated(){
                if selDay.date == day.date{
                    selectedDays.remove(at: kNum)
                }
            }
        }
    }
    
    func addDays(_ day: VADay){
        if selectedDays.contains(day){
        }else{
            selectedDays.append(day)
        }
