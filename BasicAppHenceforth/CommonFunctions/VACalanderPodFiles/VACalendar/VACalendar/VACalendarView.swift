...
public class VACalendarView: UIScrollView {
    ...
    public let calendar: VACalendar
...
//        self.contentSize = CGSize(width: UIScreen.main.bounds.width, height: 1000)
    }
...
extension VACalendarView: VAMonthViewDelegate {
    ...
    func didDeselectAll(_ day: VADay){
        calendar.setDaySelectionState(day, state: day.makeAvailableState)
        calendar.removeDays(day)
//        calendar.getSelecteddays(day, state: day.makeAvailableState)

    }
    
    func didSelectAllTheDays(_ days:[VADay]){
        for day in days{
            calendar.setDaySelectionState(day, state: day.makeSelectedState)
            calendar.addDays(day)
//            calendar.getSelecteddays(day, state: day.makeSelectedState)
        }
    }
    ...
    func dayStateChanged(_ day: VADay, in month: VAMonth) {
        ...
        case .multi:
            ...
            for (num, kMonth) in monthViews.enumerated(){
                if kMonth.month.date == month.date{
                    kMonth.changeBtnTitle(day,months: month)
                }
            }
